...
export default function LeaderboardTab({
  ...
  const { tournamentId, info, board, address, stage } = state;
  const { setActiveTab, refresh } = actions;
  ...
  if (!tournamentId)
    return (
      <div className="bg-surface rounded-2xl p-8 border border-secondary/30 text-center">
        ...
        <div className="text-xl font-medium text-[#C9E3A8] mb-2">No Tournament Selected</div>
        <div className="text-sm opacity-70 max-w-md mx-auto mb-4">Please select a tournament.</div>
      </div>
    );
  ...
  const badge =
    stage === "play" ? "bg-[#94C751] text-[#101707]" : stage === "register" ? "bg-[#F7D07A] text-[#101707]" : "bg-[#6c757d] text-white";
        ...
        <div className="bg-[#2D4014] px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <div className={`px-2 py-0.5 rounded-full text-xs font-medium ${badge}`}>
                {stage === "play" ? "Active" : stage === "register" ? "Registration" : "Ended"}
              </div>
              <button
                onClick={refresh}
                className="px-3 py-1 rounded-xl bg-[#94C751]/20 text-[#94C751] hover:bg-[#94C751]/30 text-sm inline-flex items-center gap-1"
              >
              ...
              {board.players.map((player, i) => (
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold mr-3 ${
                      i === 0
                        ...
                        : "bg-[#263711] text-highlight/70 border border-highlight/30"
                    }`}
                  >
                    {i + 1}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`truncate max-w-full ${player.toLowerCase() === address?.toLowerCase() ? "text-[#C9E3A8] font-medium" : ""}`}>
